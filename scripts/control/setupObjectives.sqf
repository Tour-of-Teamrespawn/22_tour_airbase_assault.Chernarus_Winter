["TOUR_objReturn", {"Return To Ship"}] call A2S_createSimpleTask;
["TOUR_objReturn", {"Leave the area in a helicopter to move back to <marker name=""TOUR_mkr_objReturn"">HMS Queen Elizabeth</marker>. Make sure you have smokes, and are away from the enemy. The evac choppers will fly in from the east."}, {"Return To Ship"}, {"Return To Ship"}] call A2S_setSimpleTaskDescription;
"TOUR_objReturn" call A2S_taskCommit;

["TOUR_objAirstrip", {"Capture Airstrip"}] call A2S_createSimpleTask;
["TOUR_objAirstrip", {"Assist the British paratroopers in securing <marker name=""TOUR_mkr_objAirstrip"">Olsha Airstrip</marker>, by eliminating enemy forces and armored targets, in the immediate area."}, {"Capture Airstrip"}, {"Capture Airstrip"}] call A2S_setSimpleTaskDescription;
"TOUR_objAirstrip" call A2S_taskCommit;

["TOUR_objZU23", {"Disable ZU-23"}] call A2S_createSimpleTask;
["TOUR_objZU23", {"Disable the ZU-23 to help the airborne troops to begin the assault."}, {"Disable ZU-23"}, {"Disable ZU-23"}] call A2S_setSimpleTaskDescription;
"TOUR_objZU23" call A2S_taskCommit;

["TOUR_objComms", {"Disrupt Communications"}] call A2S_createSimpleTask;
["TOUR_objComms", {"Destroy the <marker name=""TOUR_mkr_objComms"">Communication Tower</marker> to prevent distress calls."}, {"Disrupt Communications"}, {"Disrupt Communications"}] call A2S_setSimpleTaskDescription;
"TOUR_objComms" call A2S_taskCommit;
...
while {!TOUR_RC_WEST_DEAD} do
{
	if (("TOUR_objComms" call A2S_getTaskState) != "SUCCEEDED") then 
	{
		if ((isNil {missionNameSpace getVariable "TOUR_detectedTelek"})&&(isNil {missionNameSpace getVariable "TOUR_detectedArea"})) then 
		{
			...
			{
				["TOUR_objComms", "SUCCEEDED"] call A2S_setTaskState;
				"TOUR_objComms" call A2S_taskCommit;
				sleep 2;
				"TOUR_objComms" call A2S_taskHint;
				[] execVM "scripts\control\reinforceTelek.sqf";
				[] execVM "scripts\control\reinforceUral.sqf";
			};
		}else 
		{
			if (("TOUR_objComms" call A2S_getTaskState) != "FAILED") then 
			{
				sleep 3;
				["TOUR_objComms", "FAILED"] call A2S_setTaskState;
				"TOUR_objComms" call A2S_taskCommit;
				sleep 2;
				"TOUR_objComms" call A2S_taskHint;	
			};
	...
	if (("TOUR_objZU23" call A2S_getTaskState) != "SUCCEEDED") then 
	{
		if (TOUR_objZU23_start) then 
		{
			if (damage TOUR_ZU23 > 0.7) then 
			{
				["TOUR_objZU23", "SUCCEEDED"] call A2S_setTaskState;
				"TOUR_objZU23" call A2S_taskCommit;
				sleep 2;
				"TOUR_objZU23" call A2S_taskHint;
			}else 
			{
				...
				{
					if (("TOUR_objZU23" call A2S_getTaskState) != "ASSIGNED") then 
					{
						["TOUR_objZU23", "ASSIGNED"] call A2S_setTaskState;
						"TOUR_objZU23" call A2S_taskCommit;
						sleep 2;
						"TOUR_objZU23" call A2S_taskHint;
					};		
	...
	if ((!isNil {missionNameSpace getVariable "TOUR_captured"}) && (("TOUR_objAirstrip" call A2S_getTaskState) != "SUCCEEDED")) then 
	{
		["TOUR_objAirstrip", "SUCCEEDED"] call A2S_setTaskState;
		"TOUR_objAirstrip" call A2S_taskCommit;
		sleep 2;
		"TOUR_objAirstrip" call A2S_taskHint;
	};	

	if (!TOUR_RC_WEST_DEAD &&({alive _x && ((vehicle _x) distance TOUR_carrier > 100)}count (playableUnits + switchableUnits) == 0)&&(("TOUR_objReturn" call A2S_getTaskState) != "SUCCEEDED")) then 
	{
		["TOUR_objReturn", "SUCCEEDED"] call A2S_setTaskState;
		"TOUR_objReturn" call A2S_taskCommit;
		sleep 2;
		"TOUR_objReturn" call A2S_taskHint;		
	};	

	if ({canMove _x}count [TOUR_chopper_1, TOUR_chopper_2] == 0) exitWith
	{
		["TOUR_objReturn", "FAILED"] call A2S_setTaskState;
		"TOUR_objReturn" call A2S_taskCommit;
		sleep 2;
		"TOUR_objReturn" call A2S_taskHint;
		sleep 2;	
...
if ("TOUR_objAirstrip" call A2S_taskState != "SUCCEEDED") then
{
	["TOUR_objAirstrip", "FAILED"] call A2S_setTaskState;
	"TOUR_objAirstrip" call A2S_taskCommit;
};
if ("TOUR_objReturn" call A2S_taskState != "SUCCEEDED") then
{
	["TOUR_objReturn", "FAILED"] call A2S_setTaskState;
	"TOUR_objReturn" call A2S_taskCommit;
};
if ("TOUR_objZU23" call A2S_taskState != "SUCCEEDED") then
{
	["TOUR_objZU23", "FAILED"] call A2S_setTaskState;
	"TOUR_objZU23" call A2S_taskCommit;
};
if ("TOUR_objComms" call A2S_taskState != "SUCCEEDED") then
{
	["TOUR_objComms", "FAILED"] call A2S_setTaskState;
	"TOUR_objComms" call A2S_taskCommit;
};
